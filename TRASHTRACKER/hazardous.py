...
class hazardousclass:
    def __init__(self, root):
        self.root = root
        self.root.title("Eco-Friendly Hazardous Waste Management")
        self.root.geometry("1100x600+0+0")
        self.root.configure(bg="#eaf2f8")  # Light blue background for a fresh, eco-friendly feel

        # Heading
        Label(self.root, text="Manage Your Hazardous Waste Responsibly", font=("Arial", 16, "bold"), bg="#eaf2f8", fg="#2e8b57").pack(pady=10)
    ...
    def display_entries(self):
        ...
        try:
            ...
            cur.execute("SELECT * FROM category WHERE type = 'Hazardous Waste'")
            rows = cur.fetchall()
            for row in rows:
