...
class non_recyclableclass:
    def __init__(self, root):
        self.root = root
        self.root.title("Eco-Friendly Non-Recylable Waste Management")
        self.root.geometry("1100x600+0+0")
        self.root.configure(bg="#eaf2f8")  # Light blue background for a fresh, eco-friendly feel

        # Heading
        Label(self.root, text="Manage Your Non-Recylable Waste Responsibly", font=("Arial", 16, "bold"), bg="#eaf2f8", fg="#2e8b57").pack(pady=10)
        ...
        # Delete Button
        Button(self.root, text="Dispose selected trash", command=self.confirm_delete, font=("Arial", 12), bg="#e74c3c", fg="white", cursor="hand2").pack(pady=15)
    ...
    def display_entries(self):
        ...
        try:
            ...
            cur.execute("SELECT * FROM category WHERE type = 'Non-Recyclable'")
            rows = cur.fetchall()
            for row in rows:
    ...
    def confirm_delete(self):
        ...
        if not selected_item:
            messagebox.showwarning("Warning", "Please select an item to dispose", parent=self.root)
            return
